...
const svgSize: CSSProperties = { width: "280px", height: "280px" };
...
export default function SplashScreen({ onComplete }: Props) {
  ...
  useEffect(() => {
    ...
    function buildPoints(sweepDeg: number): string {
      ...
      for (let i = 0; i <= steps; i++) {
        ...
        pts.push([cx + R * Math.cos(toRad(angle)), cy + R * Math.sin(toRad(angle))]);
      }
      ...
      if (t < 1) {
        ...
        setTimeout(() => {
          if (splashRef.current) {
            ...
            splashRef.current.style.transition = `opacity ${FADE_MS}ms ease`;
          }
          setTimeout(() => { setVisible(false); onComplete(); }, FADE_MS);
        }, HOLD_MS);
      }
    }
  ...
  return (
    <div ref={splashRef} style={wrapper}>
      <svg viewBox="0 0 522 522" xmlns="http://www.w3.org/2000/svg" style={svgSize}>
        <defs>
          <clipPath id="sweep">
